...
% unrotated spiral gradient events
gx=mr.makeArbitraryGrad('x',0.99*gradSpiral(1,:),'Delay',dtDelay,'system',sys);
gy=mr.makeArbitraryGrad('y',0.99*gradSpiral(2,:),'Delay',dtDelay,'system',sys);
...
for iint=1:Nint
    ...
    seq.addBlock(mr.rotate('z', irot, gx, gy, adc));
    ...
    % Avoid pure delay block so that the gradient heating check on interpreter is accurate
